...
class BotHandlers:
    ...
    def get_start_text(self) -> str:
        return (
            ...
            f"— Сообщения в оперативной памяти хранятся <b>3 часа</b>, потом удаляются\n"
    ...
    def _referrals_text(self, user_id: int) -> str:
        ...
        if info["level"] == 1:
            next_hint = "Чтобы открыть уровень 2, пригласите <b>1 нового</b> пользователя."
        elif info["level"] == 2:
        ...
        return (
            f"{EMOJI_REFERRAL} <b>Рефералы и уровень доступа</b>\n\n"
            ...
            "Ещё закрыто: сохранение <b>одноразовых</b> медиа.\n\n"
            "<b>Уровень 3</b> — 3 и более приглашённых\n"
            "Полный доступ без ограничений, включая <b>одноразовые</b> медиа.\n\n"
            "<blockquote><i>"
            "Засчитываются только новые пользователи, "
            "которые раньше не писали /start."
            "</i></blockquote>\n\n"
            f"Ваша ссылка:\n<code>{link}</code>"
        )
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data == "info_how":
            ...
            text = (
                ...
                "<blockquote><i>"
                "После подключения вам будут автоматически приходить "
